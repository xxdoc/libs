...
- Added define for MAINPROG, and warnign disable below,
- compiled to dll and made exports __stdcall for use with VB
- added VB_SetOptions
- changed call to powl10 to powl in assembl.c (line 308)
- note: /J option sets VC default char type to unsigned

*/
#define MAINPROG
#pragma warning (disable : 4305) //dont know if this is a good idea but....
...
int    __stdcall Checkcondition(int code,ulong flags);

//this is a user defined callback, may add way to set this from vb but not yet..
...
ulong  __stdcall Disassembleback(char *block,ulong base,ulong size,ulong ip,int n);
ulong  __stdcall Disassembleforward(char *block,ulong base,ulong size,ulong ip,int n);
