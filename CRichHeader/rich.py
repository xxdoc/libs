...
def parse(fname):
    ...
    #for i in range(0, 4):
    #    print("{} {:x}".format(i,upack[i]))
        
    upack = upack[upack.index(u32(b'DanS')):]  # reverse the array
    dans = e_lfanew - len(upack) * 4 - (e_lfanew - rich)
    print("Dans offset: " + str(dans))
    ...
    for i in range(0, len(upack), 2):
        ...
        print("{} {:x} {:x} {:x}".format(i+dans, chk, upack[i + 0], upack[i + 1] ))
...
def pprint_cmpids(cmpids):
    ...
    for e in cmpids:
        print("{:s},{:s},{:s},{:s},{:s}".format(
            ...
            "{:s},{}".format(*prodids.vs_version(e['pid'])) if have_pids else '<unknown>'))
...
def pprint_header(data):
    ...
    if rich['csum_calc'] == rich['csum_file']:
        print("Checksums match! (0x{:08x})".format(rich['csum_calc']))
    else:
        print("Checksum corrupt! (calc 0x{:08x}, file "
...
if __name__ == "__main__":
    ...
    arg = "D:\_code\libs\pe_lib2\_sppe2.dll"
    rich = parse(arg)
    if rich['err'] < 0:
        print(err2str(rich['err']))
        sys.exit(rich['err'])
