...
Private Declare Function DefWindowProc Lib "user32" Alias "DefWindowProcW" (ByVal hWnd As Long, ByVal uMsg As Long, ByVal wParam As Long, ByVal lParam As Long) As Long
...
Private IPAddressHandle As Long, IPAddressEditHandle(1 To 4) As Long, IPAddressEditHandleActivate As Long
...
IPAddressHandle = CreateWindowEx(dwExStyle, StrPtr("SysIPAddress32"), StrPtr("IP Address"), dwStyle, 0, 0, UserControl.ScaleWidth, UserControl.ScaleHeight, UserControl.hWnd, 0, App.hInstance, ByVal 0&)
If IPAddressHandle <> 0 Then
...
Select Case wMsg
    Case WM_MOUSEACTIVATE
        Static InProc As Boolean
        If ComCtlsRootIsEditor(hWnd) = False And GetFocus() <> IPAddressHandle And (GetFocus() <> IPAddressEditHandle(1) Or IPAddressEditHandle(1) = 0) And (GetFocus() <> IPAddressEditHandle(2) Or IPAddressEditHandle(2) = 0) And (GetFocus() <> IPAddressEditHandle(3) Or IPAddressEditHandle(3) = 0) And (GetFocus() <> IPAddressEditHandle(4) Or IPAddressEditHandle(4) = 0) Then
            If InProc = True Then WindowProcEdit = MA_NOACTIVATEANDEAT: Exit Function
            Select Case HiWord(lParam)
                Case WM_LBUTTONDOWN, WM_RBUTTONDOWN
                    IPAddressEditHandleActivate = hWnd
                    On Error Resume Next
                    If Extender.CausesValidation = True Then
                        InProc = True
                        Screen.ActiveForm.ValidateControls
...
If wMsg = WM_SETFOCUS Then
    If IPAddressEditHandleActivate = 0 Then
        SetFocusAPI IPAddressHandle
    Else
        SetFocusAPI IPAddressEditHandleActivate
    End If
End If
End Function
