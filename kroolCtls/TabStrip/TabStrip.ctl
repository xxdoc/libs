...
PropTabFixedWidth = .ReadProperty("TabFixedWidth", 0)
PropTabFixedHeight = .ReadProperty("TabFixedHeight", 0)
PropTabMinWidth = .ReadProperty("TabMinWidth", 40)
...
On Error GoTo 0
TabStripInitTabsCount = .ReadProperty("InitTabsCount", 0)
If TabStripInitTabsCount > 0 Then
    ReDim TabStripInitTabs(1 To TabStripInitTabsCount) As InitTabStruct
    Dim i As Long
    For i = 1 To TabStripInitTabsCount
        TabStripInitTabs(i).Caption = .ReadProperty("InitTabsCaption" & CStr(i), vbNullString)
        TabStripInitTabs(i).Key = .ReadProperty("InitTabsKey" & CStr(i), vbNullString)
        TabStripInitTabs(i).Tag = .ReadProperty("InitTabsTag" & CStr(i), vbNullString)
        TabStripInitTabs(i).ToolTipText = .ReadProperty("InitTabsToolTipText" & CStr(i), vbNullString)
        TabStripInitTabs(i).Image = .ReadProperty("InitTabsImage" & CStr(i), 0)
    Next i
End If
End With
If Ambient.UserMode = True Then
...
Else
    Call CreateTabStrip
    If TabStripInitTabsCount > 0 And TabStripHandle <> 0 Then
        For i = 1 To TabStripInitTabsCount
            With Me.Tabs.Add(i, TabStripInitTabs(i).Key, TabStripInitTabs(i).Caption, TabStripInitTabs(i).Image)
            .Tag = TabStripInitTabs(i).Tag
            .ToolTipText = TabStripInitTabs(i).ToolTipText
            End With
        Next i
    End If
End If
...
.WriteProperty "TabFixedWidth", PropTabFixedWidth, 0
.WriteProperty "TabFixedHeight", PropTabFixedHeight, 0
.WriteProperty "TabMinWidth", PropTabMinWidth, 40
...
If Count > 0 Then
    ...
    For i = 1 To Count
        ...
        .WriteProperty "InitTabsImage" & CStr(i), Me.Tabs(i).Image, 0
    Next i
End If
...
Friend Sub FTabsAdd(ByVal Index As Long, Optional ByVal Caption As String, Optional ByVal Image As Long)
...
If TabStripHandle <> 0 Then
    ...
    If TabStripToolTipHandle <> 0 Then SendMessage TabStripToolTipHandle, TTM_SETMAXTIPWIDTH, 0, ByVal &H7FFF&
...
If ReInitTabsCount > 0 Then
    For i = 1 To ReInitTabsCount
        With .Tabs.Add(i, ReInitTabs(i).Key, ReInitTabs(i).Caption, ReInitTabs(i).Image)
...
If TabStripHandle <> 0 And hWnd <> 0 And hDC <> 0 Then
    ...
    ScreenToClient TabStripHandle, P
...
Private Sub GetClientRect(ByRef RC As RECT)
Dim P As POINTAPI
GetWindowRect UserControl.hWnd, RC
P.X = RC.Left
P.Y = RC.Top
ScreenToClient UserControl.ContainerHwnd, P
RC.Right = RC.Right + (P.X - RC.Left)
RC.Bottom = RC.Bottom + (P.Y - RC.Top)
RC.Left = P.X
RC.Top = P.Y
