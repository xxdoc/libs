...
Private TreeViewHandle As Long
            ...
            If Me.FNodeExpanded(.hItem) = False And PropOLEDragExpandTime > 0 Or PropOLEDragExpandTime = -1 Then
                ...
                Else
                    If TreeViewTickCount = 0 Then
                        TreeViewTickCount = CLongToULong(GetTickCount())
                    ElseIf (CLongToULong(GetTickCount()) - TreeViewTickCount) > IIf(PropOLEDragExpandTime > 0, PropOLEDragExpandTime, GetDoubleClickTime() * 2) Then
...
If Not PropNodes Is Nothing Then
    On Error Resume Next
    If UserControl.Parent Is Nothing Then Set PropNodes = Nothing
    On Error GoTo 0
End If
End Sub
...
Friend Sub FNodesAdd(ByRef NewNode As TvwNode, ByVal Key As String, Optional ByVal RelativeNode As TvwNode, Optional ByVal Relationship As TvwNodeRelationshipConstants, Optional ByVal Text As String, Optional ByVal Image As Long, Optional ByVal SelectedImage As Long)
If TreeViewHandle <> 0 Then
...
Friend Sub FNodesRemove(ByVal Handle As Long)
If TreeViewHandle <> 0 Then SendMessage TreeViewHandle, TVM_DELETEITEM, 0, ByVal Handle
End Sub
...
Friend Property Get FNodeImage(ByVal Handle As Long) As Long
If TreeViewHandle <> 0 Then
    Dim TVI As TVITEM
    With TVI
...
Friend Property Let FNodeImage(ByVal Handle As Long, ByVal Value As Long)
If TreeViewHandle <> 0 Then
    Dim TVI As TVITEM
    With TVI
    .Mask = TVIF_HANDLE Or TVIF_IMAGE
...
Friend Property Get FNodeSelectedImage(ByVal Handle As Long) As Long
If TreeViewHandle <> 0 Then
    Dim TVI As TVITEM
    With TVI
    ...
    .hItem = Handle
    SendMessage TreeViewHandle, TVM_GETITEM, 0, ByVal VarPtr(TVI)
    FNodeSelectedImage = .iSelectedImage + 1
    End With
End If
End Property

Friend Property Let FNodeSelectedImage(ByVal Handle As Long, ByVal Value As Long)
If TreeViewHandle <> 0 Then
    Dim TVI As TVITEM
    With TVI
    .Mask = TVIF_HANDLE Or TVIF_SELECTEDIMAGE
...
If TreeViewHandle <> 0 Then
    If Value = True Then
        SendMessage TreeViewHandle, TVM_EXPAND, TVE_EXPAND, ByVal Handle
    Else
        SendMessage TreeViewHandle, TVM_EXPAND, TVE_COLLAPSE, ByVal Handle
    End If
End If
End Property
...
Friend Property Get FNodeChildren(ByVal Handle As Long) As Integer
If TreeViewHandle <> 0 Then
    Dim hItem As Long, Count As Integer
...
If TreeViewHandle <> 0 Then
    Dim RC As RECT
    RC.Left = Handle
    FNodeVisible = CBool(SendMessage(TreeViewHandle, TVM_GETITEMRECT, 0, RC) <> 0)
End If
End Property
...
Friend Function FNodeCreateDragImage(ByVal Handle As Long) As Long
If TreeViewHandle <> 0 Then FNodeCreateDragImage = SendMessage(TreeViewHandle, TVM_CREATEDRAGIMAGE, 0, ByVal Handle)
End Function
...
End If
If Ambient.UserMode = True Then
    If TreeViewHandle <> 0 Then Call ComCtlsSetSubclass(TreeViewHandle, Me, 1)
End If
End Sub
...
Public Function GetFirstVisible() As TvwNode
Attribute GetFirstVisible.VB_Description = "Retrieves a reference of the first node visible in the client area."
If TreeViewHandle <> 0 Then
    Dim Ptr As Long
    Ptr = GetItemPtr(SendMessage(TreeViewHandle, TVM_GETNEXTITEM, TVGN_FIRSTVISIBLE, ByVal TVI_ROOT))
    If Ptr <> 0 Then Set GetFirstVisible = PtrToObj(Ptr)
End If
End Function
...
If TreeViewHandle <> 0 Then
    Dim Ptr As Long
    Ptr = GetItemPtr(SendMessage(TreeViewHandle, TVM_GETNEXTITEM, TVGN_CARET, ByVal TVI_ROOT))
...
If TreeViewHandle <> 0 Then
    Dim hItem As Long
    hItem = SendMessage(TreeViewHandle, TVM_GETNEXTITEM, TVGN_CARET, ByVal TVI_ROOT)
    If hItem <> 0 Then
...
Attribute SetExplorerTheme.VB_Description = "Method that gives the tree view the appearance of the windows explorer."
If TreeViewHandle <> 0 And EnabledVisualStyles() = True Then SetWindowTheme TreeViewHandle, StrPtr("Explorer"), 0
End Sub

Public Property Get LineColor() As Long
Attribute LineColor.VB_Description = "Returns/sets the line color. The value is in an RGB format. Use the CLR_DEFAULT value to restore the system default colors."
Attribute LineColor.VB_MemberFlags = "400"
If TreeViewHandle <> 0 Then LineColor = SendMessage(TreeViewHandle, TVM_GETLINECOLOR, 0, ByVal 0&)
End Property

Public Property Let LineColor(ByVal Value As Long)
If TreeViewHandle <> 0 Then SendMessage TreeViewHandle, TVM_SETLINECOLOR, 0, ByVal Value
End Property
...
If TreeViewHandle <> 0 Then
    Dim Ptr As Long
    Ptr = GetItemPtr(SendMessage(TreeViewHandle, TVM_GETNEXTITEM, TVGN_DROPHILITE, ByVal TVI_ROOT))
...
If TreeViewHandle <> 0 And Handle <> 0 And ParentHandle <> 0 Then
    ...
    .cchTextMax = 260
    End With
    SendMessage TreeViewHandle, TVM_GETITEM, 0, ByVal VarPtr(.Item)
    MoveNodes = SendMessage(TreeViewHandle, TVM_INSERTITEM, 0, ByVal VarPtr(TVIS))
    If MoveNodes <> 0 Then
        Dim Node As TvwNode
        Set Node = PtrToObj(.Item.lParam)
...
Select Case wMsg
    Case WM_SHOWWINDOW
        If TreeViewHandle = 0 Then Call CreateTreeView
    Case WM_NOTIFY
        Dim NM As NMHDR
        CopyMemory NM, ByVal lParam, LenB(NM)
        If NM.hWndFrom = TreeViewHandle Then
            Dim Node As TvwNode, TVI As TVITEM
            ...
            Select Case NM.Code
                ...
                Case NM_CLICK, NM_RCLICK
                    ...
                    If .hItem <> 0 Then
                        If (.Flags And TVHT_ONITEMBUTTON) = 0 Then
                            Set Node = PtrToObj(GetItemPtr(.hItem))
                    ...
                    Select Case NMTVCD.NMCD.dwDrawStage
                        ...
                        Case CDDS_ITEMPREPAINT
                            If NMTVCD.NMCD.lItemlParam <> 0 Then
                                ...
                                With Node
                                If (NMTVCD.NMCD.uItemState And CDIS_FOCUS) = 0 And (NMTVCD.NMCD.uItemState And CDIS_SELECTED) = 0 Then
                ...
                Case TVN_GETDISPINFO
                    ...
                    If .lParam <> 0 Then
                        Set Node = PtrToObj(.lParam)
                        If (.Mask And TVIF_IMAGE) = TVIF_IMAGE Then .iImage = Node.Image - 1
                        If (.Mask And TVIF_SELECTEDIMAGE) = TVIF_SELECTEDIMAGE Then
                            .iSelectedImage = Node.SelectedImage - 1
                            If .iSelectedImage = I_IMAGECALLBACK Then .iSelectedImage = Node.Image - 1
                        End If
                    End If
                    End With
