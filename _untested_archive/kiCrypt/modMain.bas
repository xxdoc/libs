...
' ***************************************************************************
  ...
  Public Const PGM_NAME      As String = "kiCrypt Demo"
...
Public Function ByteArrayToHex(ByRef abytData() As Byte) As String
    ...
    ByteArrayToHex = TrimStr(strHex)  ' Return hex string
...
Public Function HexToByteArray(ByVal strHex As String) As Byte()
    ...
    ' Prepare data string
    strHex = TrimStr(strHex)                      ' Remove unwanted leading/trailings chars
    strHex = Replace(strHex, " ", vbNullString)   ' Remove all blank spaces
    strHex = Replace(strHex, ",", vbNullString)   ' Remove all commas
    strHex = Replace(strHex, "&", vbNullString)   ' Remove all ampersand symbols
    strHex = Replace(strHex, "H", vbNullString)   ' Remove all "H" characters
    lngLength = Len(strHex)                       ' Capture length of data string
    ...
    If lngLength Mod 2 <> 0 Then
        lngTmpLen = (lngLength Mod 2)
        lngLength = lngLength + lngTmpLen
        strHex = Right$(String$(lngTmpLen, "0") & strHex, lngLength)
    End If

    ' String must be divisable by 2
    If lngLength Mod 2 = 0 Then

        ReDim abytData(lngLength)  ' resize output array
        lngLength = 0

        ' start converting data string two
        ' characters at a time to an ASCII
        ' decimal value
        For lngIndex = 1 To Len(strHex) Step 2
            abytData(lngLength) = CByte("&H" & Mid$(strHex, lngIndex, 2))
            lngLength = lngLength + 1
        Next lngIndex

        ReDim Preserve abytData(lngLength - 1)
    Else
        ReDim abytData(0)
    End If

    HexToByteArray = abytData()   ' Return byte array
...
Public Sub ShellAndWait(ByVal strCmdLine As String, _
    ...
    lngProcHwnd = OpenProcess(PROCESS_QUERY_INFO, False, lngProcessID)
